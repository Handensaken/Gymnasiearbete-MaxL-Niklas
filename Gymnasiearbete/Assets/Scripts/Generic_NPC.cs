...
public class Generic_NPC : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (greeted && questTracker.GetComponent<QuestTracker>().quests.ContainsKey("Greeting in order") && !questTracker.GetComponent<QuestTracker>().quests["Greetings in order"])
        {
            switch (name)
            {
